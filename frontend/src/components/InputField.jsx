const InputField = ({ label, hint, error, ...props }) => (
  <label className="flex w-full flex-col gap-2 text-sm text-[var(--mc-text)]">
    ...
    <input
      {...props}
      className="rounded-2xl border border-[var(--mc-border)] bg-white/80 px-3 py-2 sm:px-4 sm:py-3 text-sm text-[var(--mc-text)] shadow-sm outline-none transition placeholder:text-slate-400 focus:border-[var(--mc-primary)] focus:ring-4 focus:ring-[var(--mc-primary)]/10 dark:bg-[var(--mc-surface-strong)]/50"
    />
