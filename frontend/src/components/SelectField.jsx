const SelectField = ({ label, hint, error, children, ...props }) => (
  <label className="flex w-full flex-col gap-2 text-sm text-(--mc-text)">
    ...
    <select
      {...props}
      className="rounded-2xl border border-(--mc-border) bg-white/80 px-4 py-3 text-sm text-(--mc-text) shadow-sm outline-none transition focus:border-(--mc-primary) focus:ring-4 focus:ring-(--mc-primary)/10 dark:bg-(--mc-surface-strong)/50"
    >
      {children}
    </select>
